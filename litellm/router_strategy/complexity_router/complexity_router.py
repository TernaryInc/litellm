...
class ComplexityRouter(CustomLogger):
    ...
    def _resolve_advisor_model(self, configured: str) -> str | None:
        ...
        A name serving router deployments resolves through them (a model-group name
        forwarded verbatim is rejected upstream, LIT-6020); a name matching no
        deployment passes through as a raw Anthropic model id. Fails closed with None
        when any deployment behind the name is non-Anthropic, when the group's
        deployments declare more than one distinct underlying model (configuration
        order must never pick the consulted model), or when a wildcard survives
        resolution: the pattern router substitutes matched segments into the model,
        so a surviving wildcard means an exact-named group with a literal wildcard
        model, which nothing can turn into a sendable model id.
        """
    ...
    def _advisor_augmented_params(
        ...
        Fail-closed: the tool is injected only for a non-streaming request whose tier
        model group resolves entirely to Anthropic deployments (the only provider that
        orchestrates advisor_20260301 natively) and whose tools list does not already
        carry an advisor tool. Every skip reports advisor_offered=False on the routing
