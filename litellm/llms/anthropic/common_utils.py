...
def build_advisor_tool(
    ...
    advisor_tool: Final[AnthropicAdvisorTool] = {
...
def strip_advisor_blocks_from_content(content: Sequence[Any], replace_with_text: bool = False) -> Sequence[Any] | None:
    ...
    server_tool_use blocks. In replace mode the exchange collapses into an
    <advisor_feedback> text block so the guidance survives as ordinary text;
    otherwise it is stripped silently. advisor_tool_result blocks paired with
    a matched server_tool_use are always dropped.
    """
...
def stripped_response_content_for_injected_advisor(content: object) -> Sequence[Any] | None:
    ...
    Returns the rewritten content-block list (advice preserved as <advisor_feedback>
    text so a caller replaying the history keeps the plan), or None when the response
    carries no advisor exchange. A plain tool_use named advisor means the upstream
    account never orchestrated the consult (advisor beta not enabled); it is left
    untouched, because stripping it would orphan the response's stop_reason.
    """
    ...
    if rewritten is None:
        if any(
            isinstance(block, dict) and block.get("type") == "tool_use" and block.get("name") == "advisor"
            for block in content
        ):
            litellm.verbose_logger.warning(
                "Advisor tool_use came back unorchestrated (the upstream account may lack the advisor beta); "
