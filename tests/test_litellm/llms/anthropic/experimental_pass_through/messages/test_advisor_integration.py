...
async def test_finalize_strips_router_injected_advisor_blocks():
    ...
    result = await handler._finalize_anthropic_messages_response(
        ...
        custom_llm_provider="anthropic",
        api_key=None,
        kwargs={ADVISOR_INJECTED_PARAM: True},
    )
...
async def test_finalize_keeps_advisor_blocks_without_marker():
    ...
    assert [block["type"] for block in result["content"]] == ["server_tool_use", "advisor_tool_result"]
