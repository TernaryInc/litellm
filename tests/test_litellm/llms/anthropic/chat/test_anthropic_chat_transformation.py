...
def _run_transform_response(litellm_params: dict):
    import httpx
    ...
    config = AnthropicConfig()
    raw_response = httpx.Response(status_code=200, headers={}, json=_advised_completion_response())
...
def test_transform_response_leaves_unorchestrated_advisor_tool_use():
    ...
    raw_response = httpx.Response(status_code=200, headers={}, json=unorchestrated)
    result = config.transform_response(
        ...
        messages=[{"role": "user", "content": "Redis or Postgres?"}],
        optional_params={},
        litellm_params={ADVISOR_INJECTED_PARAM: True},
        encoding=None,
    )
