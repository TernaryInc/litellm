...
class TestAdvisorTiers:
    ...
    @staticmethod
    def _hook_router(config_tiers: Dict, **config_extra) -> ComplexityRouter:
        deployments = {
    ...
    async def test_advisor_tool_is_appended_to_caller_tools(self):
        """The core workflow: the caller's own tools survive byte-identical, the advisor tool is
        appended with the model resolved through the router (never the group name verbatim), and
        the injected-marker rides the request for the response strip."""
    ...
    async def test_streaming_request_is_not_advised(self):
        """The response strip is non-streaming only, so a streaming request must reach the
        provider without the tool rather than leak advisor blocks into the caller's stream."""
        router = Router(model_list=self._advised_model_list())
    ...
    async def test_caller_supplied_advisor_tool_is_left_untouched(self):
        """A caller who brought their own advisor keeps it: no double tool, no marker, so the
        response strip never touches blocks the caller asked for."""
        router = Router(model_list=self._advised_model_list())
    ...
    async def test_decision_reports_advisor_and_declared_params_not_merged_tools(self):
        """The routing decision names the advisor outcome and the operator's declared tier params;
        the caller's merged tool list stays off the record, or every agentic request would write
        its full tool schemas into the spend log."""
    ...
    async def test_tier_declared_tools_stay_deliberate_overrides_and_gain_the_advisor(self):
        """Tier litellm_params are deliberate overrides (same precedence as every tier param, e.g.
        reasoning_effort): declared tier tools replace the caller's, and the advisor appends to the
        list that actually ships. This pins the pre-existing carrier semantics rather than merging
        two tool sources nobody declared together."""
    ...
    async def test_advisor_group_with_one_underlying_model_across_deployments_resolves(self):
        deployments = {
            "haiku-tier": [{"model_name": "haiku-tier", "litellm_params": {"model": "anthropic/claude-haiku-4-5"}}],
            "opus-pool": [
                {"model_name": "opus-pool", "litellm_params": {"model": "anthropic/claude-opus-5"}},
                {"model_name": "opus-pool", "litellm_params": {"model": "anthropic/claude-opus-5"}},
            ],
        }
        instance = MagicMock()
        ...
        router = ComplexityRouter(
            ...
            litellm_router_instance=instance,
            complexity_router_config={
                "tiers": {"SIMPLE": {"model_name": "haiku-tier", "advisor": {"model": "opus-pool"}}}
            },
        )
    ...
    async def test_advisor_group_with_distinct_underlying_models_is_skipped(self):
        ...
        deployments = {
            ...
            "opus-pool": [
                {"model_name": "opus-pool", "litellm_params": {"model": "anthropic/claude-opus-5"}},
                {"model_name": "opus-pool", "litellm_params": {"model": "anthropic/claude-opus-4-6"}},
            ],
        }
        instance = MagicMock()
        instance.get_model_list = lambda model_name=None, team_id=None: deployments.get(model_name, [])
    ...
    async def test_wildcard_served_advisor_name_resolves_and_literal_wildcard_fails_closed(self):
        """A model id served through a wildcard deployment resolves concretely, because the
        pattern router substitutes the matched segments into the deployment model. A wildcard
        that survives resolution is an exact-named group with a literal wildcard model, which
        nothing can turn into a sendable id, so it fails closed."""
        real_router = Router(
            model_list=[
        ...
        deployments = {
            "opus-alias": [{"model_name": "opus-alias", "litellm_params": {"model": "anthropic/claude-*"}}],
        }
        instance = MagicMock()
        instance.get_model_list = lambda model_name=None, team_id=None: deployments.get(model_name, [])
        instance.cache = DualCache()
        literal_wildcard = ComplexityRouter(
            model_name="test-router",
            litellm_router_instance=instance,
            complexity_router_config={
                "tiers": {"SIMPLE": {"model_name": "haiku-tier", "advisor": {"model": "opus-alias"}}}
            },
        )
        assert literal_wildcard._resolve_advisor_model("opus-alias") is None
